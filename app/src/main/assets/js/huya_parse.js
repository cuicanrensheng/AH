function parseHuya(url) {
    try {
        var roomId = '';
        var match = url.match(/(\d+)/);
        if (match) {
            roomId = match[1];
        }
        
        if (!roomId) {
            return JSON.stringify({error: '无法获取房间ID'});
        }
        
        var apiUrl = 'https://www.huya.com/' + roomId;
        var headers = JSON.stringify({
            'User-Agent': 'Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36',
            'Referer': 'https://www.huya.com/'
        });
        
        var html = getUrlContent(apiUrl, headers);
        if (!html) {
            return JSON.stringify({error: '获取页面失败'});
        }
        
        // 尝试解析 streamInfo 格式
        var streamInfoMatch = html.match(/var\s+streamInfo\s*=\s*({[\s\S]*?});/);
        if (streamInfoMatch) {
            try {
                var streamInfo = JSON.parse(streamInfoMatch[1]);
                var result = [];
                
                if (streamInfo.data && streamInfo.data.streamList) {
                    streamInfo.data.streamList.forEach(function(stream) {
                        result.push({
                            quality: stream.sName,
                            url: stream.sFlvUrl + '/' + stream.sFlvAntiCode + '.flv'
                        });
                    });
                }
                
                return JSON.stringify({success: true, data: result});
            } catch (e) {
                return JSON.stringify({error: '解析失败: ' + e.message});
            }
        }
        
        // 尝试解析 window.liveData 格式
        var liveDataMatch = html.match(/window\.liveData\s*=\s*({[\s\S]*?});/);
        if (liveDataMatch) {
            try {
                var liveData = JSON.parse(liveDataMatch[1]);
                var result = [];
                
                if (liveData.stream && liveData.stream.streamList) {
                    liveData.stream.streamList.forEach(function(stream) {
                        result.push({
                            quality: stream.sName,
                            url: stream.sFlvUrl + '/' + stream.sFlvAntiCode + '.flv'
                        });
                    });
                }
                
                return JSON.stringify({success: true, data: result});
            } catch (e) {
                return JSON.stringify({error: '解析失败: ' + e.message});
            }
        }
        
        // 尝试解析 __INITIAL_STATE__ 格式
        var initialStateMatch = html.match(/window\.__INITIAL_STATE__\s*=\s*({[\s\S]*?});/);
        if (initialStateMatch) {
            try {
                var state = JSON.parse(initialStateMatch[1]);
                var result = [];
                
                if (state.roomInfo && state.roomInfo.tLiveInfo && state.roomInfo.tLiveInfo.tLiveStreamInfo) {
                    var streamInfo = state.roomInfo.tLiveInfo.tLiveStreamInfo;
                    if (streamInfo.vMultiStreamInfo) {
                        streamInfo.vMultiStreamInfo.forEach(function(stream) {
                            if (stream.sHlsUrl) {
                                result.push({
                                    quality: 'hls',
                                    url: stream.sHlsUrl
                                });
                            }
                            if (stream.sFlvUrl) {
                                result.push({
                                    quality: stream.sName || 'flv',
                                    url: stream.sFlvUrl
                                });
                            }
                        });
                    }
                }
                
                if (result.length > 0) {
                    return JSON.stringify({success: true, data: result});
                }
            } catch (e) {
                // 继续尝试其他格式
            }
        }
        
        // 尝试从 hyPlayerConfig 提取
        var configMatch = html.match(/hyPlayerConfig\s*=\s*({[\s\S]*?});/);
        if (configMatch) {
            try {
                var config = JSON.parse(configMatch[1]);
                var result = [];
                
                if (config.gameLiveInfo && config.gameLiveInfo.liveStreamInfo) {
                    var streamInfo = config.gameLiveInfo.liveStreamInfo;
                    if (streamInfo.sFlvUrl) {
                        result.push({
                            quality: 'flv',
                            url: streamInfo.sFlvUrl + (streamInfo.sFlvAntiCode ? '?' + streamInfo.sFlvAntiCode : '')
                        });
                    }
                    if (streamInfo.sHlsUrl) {
                        result.push({
                            quality: 'hls',
                            url: streamInfo.sHlsUrl
                        });
                    }
                }
                
                if (result.length > 0) {
                    return JSON.stringify({success: true, data: result});
                }
            } catch (e) {
                // 继续
            }
        }
        
        return JSON.stringify({error: '未找到直播信息'});
    } catch (e) {
        return JSON.stringify({error: '异常: ' + e.message});
    }
}

function getParseResult(funcName, url) {
    return parseHuya(url);
}
