...
static void key_expansion_256(const uint8_t key[32], uint8_t out[240]) {
    ...
    while (i < 60) {
        ...
        for (int j = 0; j < 4; ++j) out[i * 4 + j] = out[(i - 8) * 4 + j] ^ temp[j];
        ++i;
    }
}
...
static void inv_cipher_block(uint8_t in[16], uint8_t out[16], const uint8_t* round_keys) {
    uint8_t state[16];
    memcpy(state, in, 16);
...
extern "C" int aes256_cbc_decrypt(const uint8_t* in, int in_len,
    ...
    if (in_len < 32 || (in_len % 16) != 0) return -2;
    ...
    int data_len = in_len - 16;
    int out_cap = data_len;
    *out_len = 0;

    uint8_t prev[16];
    memcpy(prev, iv, 16);

    for (int off = 16; off < in_len; off += 16) {
        uint8_t dec[16];
        inv_cipher_block((uint8_t*)(in + off), dec, round_keys);
        for (int j = 0; j < 16; ++j) dec[j] ^= prev[j];
        if (out_cap - *out_len < 16) return -3;
        memcpy(out + *out_len, dec, 16);
        *out_len += 16;
