...
public class EpgManager {
    ...
    private void parseXml(InputStream is) throws Exception {
        ...
        maxDate.add(Calendar.DAY_OF_YEAR, 7);   // 🔧 扩大范围：允许未来7天的节目（日期列表现在覆盖到+7）
            ...
            if (xml.getEventType() == XmlPullParser.START_TAG) {
                ...
                if ("programme".equals(tag)) {
                    ...
                    try {
                        ...
                        int offsetMinutes = 8 * 60;   // 默认：北京时间 (UTC+8)
    ...
    public String getDayName(Calendar itemCal, Calendar todayCal) {
        ...
        twoDaysAgo.setTime(todayDay.getTime());
        twoDaysAgo.add(Calendar.DAY_OF_YEAR, -2);
        if (itemDay.get(Calendar.YEAR) == twoDaysAgo.get(Calendar.YEAR)
                && itemDay.get(Calendar.DAY_OF_YEAR) == twoDaysAgo.get(Calendar.DAY_OF_YEAR)) {
            return "前天";
        }

        // 🔧 新增：昨天（today - 1）
        Calendar yesterday = Calendar.getInstance();
        yesterday.setTime(todayDay.getTime());
        yesterday.add(Calendar.DAY_OF_YEAR, -1);
        if (itemDay.get(Calendar.YEAR) == yesterday.get(Calendar.YEAR)
                && itemDay.get(Calendar.DAY_OF_YEAR) == yesterday.get(Calendar.DAY_OF_YEAR)) {
            return "昨天";
        }
