...
public class EpgManager {
    ...
    public static EpgManager getInstance(Context ctx) {
        if (instance == null) instance = new EpgManager(ctx.getApplicationContext());
        return instance;
    }

    public static EpgManager getInstance() {
        if (instance == null) throw new RuntimeException("EpgManager not init");
        return instance;
    }
    ...
    public void setEpgUrl(String url) { this.epgUrl = url; }

    public void loadEpgFromM3u(String m3uUrl, Runnable callback) {
        new Thread(() -> {
            String extractedEpgUrl = extractEpgUrlFromM3u(m3uUrl);
            if (extractedEpgUrl != null && !extractedEpgUrl.isEmpty()) epgUrl = extractedEpgUrl;
            loadEpg(callback);
        }).start();
    }
    ...
    private String extractEpgUrlFromM3u(String m3uUrl) {
        ...
        try (okhttp3.Response response = com.tv.live.util.NetUtil.getInstance().syncGet(m3uUrl)) {
            ...
            if (m3uUrl.endsWith(".gz")) is = new GZIPInputStream(is);
            ...
            while ((line = reader.readLine()) != null && lineCount < 10) {
                ...
                if (line.contains("x-tvg-url") || line.contains("tvgtvg-url")) {
                    ...
                    if (start >= 0) {
                        ...
                        if (end > start) return line.substring(start + 1, end).trim();
                    }
        ...
        } finally { try { if (reader != null) reader.close(); } catch (Exception ignored) {} }
        return null;
    }
        ...
        new Thread(() -> {
            try (okhttp3.Response response = com.tv.live.util.NetUtil.getInstance().syncGet(epgUrl)) {
                ...
                try {
                    ...
                    if (cacheIs == null) return;
                    try { parseXml(cacheIs); } finally { cacheIs.close(); }
    ...
    private void parseXml(InputStream is) throws Exception {
        ...
        while (xml.getEventType() != XmlPullParser.END_DOCUMENT) {
            if (xml.getEventType() == XmlPullParser.START_TAG) {
                ...
                if ("display-name".equals(tag)) currentChannelName = xml.nextText().trim();
                if ("programme".equals(tag)) {
                    ...
                    try {
                        ...
                        if (startCal.after(maxDate)) continue;
    ...
    public List<Channel.EpgItem> getEpg(String channelName) {
        ...
        if (channelEpgMap.containsKey(channelName)) return channelEpgMap.get(channelName);
        String cleanName = normalizeChannelName(channelName);
        String bestMatch = null; int bestScore = 0;
        for (Map.Entry<String, List<Channel.EpgItem>> entry : channelEpgMap.entrySet()) {
            ...
            if (score > bestScore) { bestScore = score; bestMatch = epgName; }
        }
        if (bestMatch != null && bestScore >= 20) return channelEpgMap.get(bestMatch);
    ...
    public interface OnHuyaEpgReadyListener { void onHuyaEpgReady(Channel channel); }
    ...
    private static String channelKey(String channelName) { return "name:" + (channelName == null ? "" : channelName); }
    ...
    public void startPlayback(Channel ch, String programTitle) {
        ...
        if (isHuya) notifyHuyaEpgReady(ch);
    }
    ...
    private List<Channel.EpgItem> buildHistoryEpgFromStore(String key, String fallbackName) {
        ...
        for (PlaybackSegment seg : segments) {
            ...
            String endHm = isPlaying ? "直播中" : sdfHm.format(new java.util.Date(seg.endTimeMs));
            result.add(new Channel.EpgItem("今天", startHm + " - " + endHm, seg.title, isPlaying));
        }
        return result;
    }
    ...
    private String normalizeChannelName(String name) {
        ...
        if (normalizedNameCache.containsKey(name)) return normalizedNameCache.get(name);
    ...
    private int calculateMatchScore(String s1, String s2) {
        ...
        for (int i = 0; i < minLen; i++) {
            if (s1.charAt(i) == s2.charAt(i)) prefixLen++; else break;
        }
        if (prefixLen >= 2) return prefixLen * 5;
        return 0;
    }
    ...
    public String getDayName(Calendar itemCal, Calendar todayCal) {
        ...
        itemDay.set(Calendar.HOUR_OF_DAY, 0); itemDay.set(Calendar.MINUTE, 0);
        itemDay.set(Calendar.SECOND, 0); itemDay.set(Calendar.MILLISECOND, 0);
        Calendar todayDay = Calendar.getInstance();
        todayDay.setTime(todayCal.getTime());
        todayDay.set(Calendar.HOUR_OF_DAY, 0); todayDay.set(Calendar.MINUTE, 0);
        todayDay.set(Calendar.SECOND, 0); todayDay.set(Calendar.MILLISECOND, 0);
        if (itemDay.get(Calendar.YEAR) == todayDay.get(Calendar.YEAR)
                && itemDay.get(Calendar.DAY_OF_YEAR) == todayDay.get(Calendar.DAY_OF_YEAR)) return "今天";
        Calendar tomorrow = Calendar.getInstance();
        tomorrow.setTime(todayDay.getTime()); tomorrow.add(Calendar.DAY_OF_YEAR, 1);
        if (itemDay.get(Calendar.YEAR) == tomorrow.get(Calendar.YEAR)
                && itemDay.get(Calendar.DAY_OF_YEAR) == tomorrow.get(Calendar.DAY_OF_YEAR)) return "明天";
        Calendar dayAfter = Calendar.getInstance();
        dayAfter.setTime(todayDay.getTime()); dayAfter.add(Calendar.DAY_OF_YEAR, 2);
        if (itemDay.get(Calendar.YEAR) == dayAfter.get(Calendar.YEAR)
                && itemDay.get(Calendar.DAY_OF_YEAR) == dayAfter.get(Calendar.DAY_OF_YEAR)) return "后天";
        String[] weekDays = {"周日", "周一", "周二", "周三", "周四", "周五", "周六"};
        int dayOfWeek = itemCal.get(Calendar.DAY_OF_WEEK) - 1;
        return weekDays[dayOfWeek];
    }

    public int getChannelEpgMapSize() { return channelEpgMap.size(); }
