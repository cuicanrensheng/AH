...
public class MainActivity extends AppCompatActivity {
        ...
        appCoreManager.setOnDataLoadListener(new AppCoreManager.OnDataLoadListener() {
            ...
            public void onLiveSourceLoaded(List<Channel> channels, boolean fromCache) {
                ...
                mMainHandler.post(() -> {
                    ...
                    if (currentPlayIndex >= 0 && currentPlayIndex < channelSourceList.size()) {
                        Channel ch = channelSourceList.get(currentPlayIndex);
                        // 🔧 修复"回前台卡顿"：网络源加载完成时，如果播放器正在播放同一频道，
                        // 跳过重新 doPlay，避免重复缓冲造成画面卡顿（缓存已触发过首播）。
                        if (!fromCache && mPlayerManager != null
                                && mPlayerManager.getCurrentChannel() != null
                                && mPlayerManager.getCurrentChannel().getName() != null
                                && mPlayerManager.getCurrentChannel().getName().equals(ch.getName())
                                && mPlayerManager.isPlaying()) {
                            log("【网络】播放器已在播放该频道，跳过重复 doPlay，避免卡顿");
                        } else if (ch != null && !TextUtils.isEmpty(ch.getPlayUrl())) {
                            // 🔧 修复"冷启动卡顿"：上次播放的是虎牙频道时，首播会触发实时解析，
                            // 首帧需 5~7 秒（即使命中预解析缓存也要等 SDK 取流）。
                            // 改为跳过虎牙频道，优先播放第一个普通源频道（700ms 首帧），
                            // 用户仍可在频道面板手动切回虎牙频道。
                            if (ch.getHuyaRoomId() > 0 && fromCache) {
                                Channel firstNonHuya = findFirstNonHuyaChannel(channelSourceList);
                                if (firstNonHuya != null) {
                                    currentPlayIndex = channelSourceList.indexOf(firstNonHuya);
                                    appConfig.setLastPlayIndex(currentPlayIndex);
                                    channelPanelController.setCurrentPlayIndex(currentPlayIndex);
                                    log("【缓存】上次为虎牙频道，改播普通源 #" + currentPlayIndex + "：" + firstNonHuya.getName());
                                    playChannel(firstNonHuya, currentPlayIndex);
                                } else {
                                    log("【缓存】自动播放频道 #" + currentPlayIndex + "：" + ch.getName());
    ...
    /**
     * 查找第一个非虎牙且有效播放地址的频道。
     * 用于冷启动时跳过虎牙频道（虎牙首帧需 5~7 秒实时解析），改播普通源（约 700ms 首帧）。
     */
    private Channel findFirstNonHuyaChannel(List<Channel> channels) {
        if (channels == null || channels.isEmpty()) return null;
        for (Channel c : channels) {
            if (c == null) continue;
            if (c.getHuyaRoomId() > 0) continue;            // 跳过虎牙频道
            if (TextUtils.isEmpty(c.getPlayUrl())) continue; // 跳过无播放地址
            return c;
        }
        return null;
    }
