...
/**
 * 直播主界面
 * 核心功能：直播播放、频道切换、EPG节目单、侧边面板
 * 本次修复重点：
 * 1. 移除【切换控制器】广播（该广播会强制开启控制器，是弹窗漏洞）
 * 2. 修复触摸事件漏洞：面板打开也完全拦截，不传递给原生View
 * 3. 删除低版本不存在的API，彻底解决编译报错
 * 4. 多层加固原生控制器屏蔽，杜绝各种场景下弹窗
 * 5. 保留所有原有业务逻辑（切台、EPG、手势、按键）
 */
public class MainActivity {
    ...
    // ===================== 【修复】已移除危险广播：切换控制器广播 =====================
    // 原 toggleControllerReceiver 广播会强制开启原生控制器，彻底删除
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 【已删除】setShowPlayButton / setShowSubtitleButton（低版本无此方法，编译报错）
    ...
    public void onReceiveConfig(String liveUrl, String epgUrl) {
        appConfig.setCustomUrls(liveUrl);
        if (liveUrl != null) UrlConfig.LIVE_URL = liveUrl;
        if (epgUrl != null) UrlConfig.EPG_URL = liveUrl;
