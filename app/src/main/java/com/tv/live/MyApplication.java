...
public class MyApplication extends Application {
    ...
    private volatile boolean huyaSdkInited = false;
    ...
    private void initializeAsync() {
        ...
        // 延迟 2 秒后初始化日志/缓存/设备能力（轻量，不阻塞 UI）
        // 虎牙 SDK 改由 onPlayReady 触发，避免和首帧抢主线程
    ...
    /**
     * 由 MainActivity 在播放器 STATE_READY 后调用，在主线程初始化虎牙 SDK。
     * 虎牙 SDK 的 HyMars.<init> 强依赖主线程 Looper，不能放后台线程；
     * 此时首帧已渲染完成，主线程短暂阻塞 ~300ms 不会被用户感知为"启动卡顿"。
     */
    public void initHuyaSDKOnPlayReady() {
        if (huyaSdkInited) return;
        huyaSdkInited = true;
        mainHandler.post(this::initHuyaSDK);
    }
