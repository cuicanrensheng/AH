...
public class SettingsDialog extends android.app.Dialog {
    ...
    private void showDebugLogDialog() {
        ...
        // 6. 绑定两步确认
        Button finalBtnStartStop = btnStartStop;
        Button finalBtnClose = btnClose;

        setupTwoStepButton(finalBtnStartStop, () -> {
            boolean newState = !currentDebugState;
            sp.edit().putBoolean(KEY_DEBUG_LOG_ENABLE, newState).apply();
            finalBtnStartStop.setText(newState ? "停止记录" : "开始记录");
            finalBtnStartStop.setTextColor(Color.BLACK);
            updateDebugLogStatus();
            if (!newState) {
        ...
        setupTwoStepButton(finalBtnClose, () -> {
            if (dialog != null) {
                dialog.dismiss();
            }
        }, Color.BLACK);
    ...
    private void initSettingsItemList() {
        ...
        View[] items = {
            findViewById(R.id.item_boot),           // 0
            findViewById(R.id.item_reverse),        // 1
            findViewById(R.id.item_pip),            // 2
            findViewById(R.id.item_channel_line),   // 3
            findViewById(R.id.item_resolution),     // 4
            findViewById(R.id.item_decoder),        // 5
            findViewById(R.id.item_renderer),       // 6
            findViewById(R.id.tv_screen_ratio),     // 7
            findViewById(R.id.item_redirect),       // 8
            findViewById(R.id.item_live_subscribe),  // 9
            findViewById(R.id.item_epg_subscribe),   // 10
            itemTifSync,                            // 11
            findViewById(R.id.item_check_update),   // 12
            findViewById(R.id.item_version_info),   // 13
            itemDebugLog,                            // 14
            findViewById(R.id.item_background_switch),   // 15
            findViewById(R.id.item_exit_dialog)      // 16
        };
    ...
    private void setupTwoStepButton(Button button, Runnable action, int normalTextColor) {
        ...
        button.setOnKeyListener((v, keyCode, event) -> {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                ...
                } else if (keyCode == KeyEvent.KEYCODE_BACK && pending[0]) {
                    pending[0] = false;
                    return true;
                }
            }
