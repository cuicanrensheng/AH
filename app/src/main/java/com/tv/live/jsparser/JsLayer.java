...
public class JsLayer {
    ...
    public static void init(Context context) {
        ...
        filesDirPath = ctx.getFilesDir().getPath() + "/";
        webView = newWebView;
    ...
    public static void evaluate(String str, final JsCallback jsCallback) {
        ...
        try {
            ...
            final WebView finalWv = webView;
            // 🟢 修复：用局部变量保存callback，避免静态pendingCallback被并发覆盖
            final JsCallback localCallback = jsCallback;
            finalWv.post(() -> {
                finalWv.evaluateJavascript(fullScript, value -> {
                    // 🟢 修复：只移除WebView添加的外层引号，保留JSON内部引号
                    if (value != null && !"null".equals(value)) {
                        String parsedValue = value;
                        // WebView evaluateJavascript对字符串返回 "xxx"（带外层双引号），需移除
                        ...
                        localCallback.onResult(parsedValue);
                    } else {
                        localCallback.onError("no result");
                    }
                });
            });
        } catch (Exception e) {
            jsCallback.onError("internal error: " + e.getMessage());
        }
    }
    ...
    public static String assetFileToString(String str) {
        try {
            if (assetManager == null && appContext != null) {
