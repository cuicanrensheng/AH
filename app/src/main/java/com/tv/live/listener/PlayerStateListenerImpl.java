...
public class PlayerStateListenerImpl implements TVPlayerManager.OnPlayStateListener {
    ...
    public void onPlayReady() {
        // 播放就绪无额外UI操作
        // 首帧渲染完成后触发虎牙 SDK 初始化，避免和启动首帧抢主线程时间片
        if (context instanceof android.app.Application) {
            // 构造时已转 applicationContext，这里实际就是 MyApplication 实例
            ((MyApplication) context).initHuyaSDKOnPlayReady();
        } else {
            android.app.Application app = (android.app.Application) context.getApplicationContext();
            if (app instanceof MyApplication) {
                ((MyApplication) app).initHuyaSDKOnPlayReady();
            }
        }
    }
