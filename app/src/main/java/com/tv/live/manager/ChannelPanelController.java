...
public class ChannelPanelController {
                    ...
                    panelLayout.post(() -> {
                        try {
                            if (epgManagerWrapper != null) {
                                ...
                                android.util.Log.d("ChannelPanel", "🟢【虎牙EPG刷新】解析完成，已刷新右侧面板："
    ...
    public void setChannels(List<Channel> channels) {
        ...
        android.util.Log.d("ChannelPanel", "setChannels: 原始频道数=" + channels.size());
        ...
        // 🟢 异步加载"虎牙游戏直播"（SDK 游戏分类推荐），独立成组，与一起看区分
        loadHuyaGameChannels();
    }
        ...
        HuyaTogetherWatchManager.getInstance().fetchTogetherWatchChannels(
            ...
            public void onSuccess(List<Channel> channels) {
                if (channels == null || channels.isEmpty()) {
                    Log.d("ChannelPanel", "虎牙一起看: 未获取到频道");
                    return;
                }
                panelLayout.post(() -> {
                    ...
                    Log.d("ChannelPanel", "虎牙一起看加载完成，新增 " + added + " 个频道，总计 " + channelSourceList.size());
                    if (!twRoomIds.isEmpty()) {
                        Log.d("ChannelPanel", "🟢【虎牙预解析】(一起看) 收集到 " + twRoomIds.size() + " 个房间，开始后台并行预解析");
            ...
            public void onFailed(String errorMsg) {
                Log.d("ChannelPanel", "加载虎牙一起看失败: " + errorMsg);
            }
        });
    }

    /**
     * 异步加载"虎牙游戏直播"（SDK 游戏分类推荐：王者荣耀/英雄联盟等）并追加到频道列表，
     * 独立成"虎牙游戏直播"组，与"一起看"(虎牙影视)区分，不混入影视分类。
     */
    private void loadHuyaGameChannels() {
        HuyaTogetherWatchManager.getInstance().fetchGameChannels(
                new HuyaTogetherWatchManager.OnChannelsFetchedListener() {
            @Override
            public void onSuccess(List<Channel> channels) {
                if (channels == null || channels.isEmpty()) {
                    Log.d("ChannelPanel", "虎牙游戏直播: 未获取到频道");
                    return;
                }
                panelLayout.post(() -> {
                    if (activity == null || activity.isFinishing() || activity.isDestroyed()) return;
                    ...
                    int added = 0;
                    java.util.ArrayList<Integer> gameRoomIds = new java.util.ArrayList<>();
                    for (Channel c : channels) {
                        if (c.getChannelId() != null && !existIds.contains(c.getChannelId())) {
                            channelSourceList.add(c);
                            existIds.add(c.getChannelId());
                            added++;
                            if (c.getHuyaRoomId() > 0) gameRoomIds.add(c.getHuyaRoomId());
                        }
                    }
                    Log.d("ChannelPanel", "虎牙游戏直播加载完成，新增 " + added + " 个频道，总计 " + channelSourceList.size());
                    if (!gameRoomIds.isEmpty()) {
                        Log.d("ChannelPanel", "🟢【虎牙预解析】(游戏直播) 收集到 " + gameRoomIds.size() + " 个房间，开始后台并行预解析");
                        com.tv.live.util.HuyaSDKParser.preloadRooms(gameRoomIds);
                    }
                    groupListManager.setGroups(channelSourceList);
                    currentGroupName = GroupListManager.GROUP_ALL;
                    currentGroupChannelList.clear();
            ...
            @Override
            public void onFailed(String errorMsg) {
                Log.d("ChannelPanel", "加载虎牙游戏直播失败: " + errorMsg);
            }
        });
    }
