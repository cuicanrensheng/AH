...
public final class DeviceCapabilities {
    ...
    /**
     * 确保检测已执行（懒加载，异步执行，不阻塞调用线程）。
     * ⚡【启动加速】之前是 synchronized 同步在调用线程 detect() → MediaCodecList/硬件探测 50~200ms 阻塞主线程
     *   现在：如果还未检测 → 立刻 return，丢到独立后台线程异步 detect()
     *   首次使用 isTv()/isOldTvChipset()/getPreferredTargetHeight() 会拿到合理默认值，
     *   检测完成后 volatile 字段自动刷新，后续调用就是正确值。
     */
    public static void ensureDetected(final Context appContext) {
        if (sDetected) return;
        final Object lock = DeviceCapabilities.class;
        synchronized (lock) {
            if (sDetected) return;
            // ⚡ 不阻塞调用者：直接后台线程跑，不 return 任何结果给当前线程
            new Thread(new Runnable() {
                @Override public void run() {
                    try {
                        ...
                        android.util.Log.w(TAG, "ensureDetected async failed: " + t.getMessage());
                    }
                }
            }, "device-cap-detect").start();
        }
    }
