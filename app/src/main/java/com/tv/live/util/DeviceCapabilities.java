...
public final class DeviceCapabilities {
     ...
     * 当前条件：老电视芯片 → 强制软解。
     *
     * <p>注意：此方法返回是否使用 MediaCodec 软解器（如 OMX.google、c2.android）。
     * 新版设备推荐使用 {@link #shouldUseFfmpeg()} 来判断是否使用 FFmpeg 软解，
     * FFmpeg 软解比 Android 系统软解器效率更高、支持格式更广。
     */
    ...
    /**
     * 是否应该使用 FFmpeg 软解（推荐替代方案）。
     *
     * <p>触发条件（满足任一）：
     * <ul>
     *   <li>老电视芯片（MStar/MT9255）— 硬解 1080p 不稳定或无法处理高分辨率</li>
     *   <li>硬解器黑名单 — 已知不支持高分辨率或有编码缺陷的硬解器</li>
     * </ul>
     *
     * <p>FFmpeg 软解优势：
     * <ul>
     *   <li>H.264/H.265/AAC 等主流格式均由 FFmpeg libavcodec 处理</li>
     *   <li>不依赖系统 MediaCodec 实现，绕过厂商 bug</li>
     *   <li>跨平台一致性，同一 APK 在所有设备上行为相同</li>
     * </ul>
     */
    public static boolean shouldUseFfmpeg() {
        if (sIsOldTvChipset) {
            return true;
        }
        // 硬解器黑名单（已知缺陷）：
        //   OMX.MS.AVC.Decoder (MStar) — 1080p 超时
        //   c2.amlogic.avc.decoder.awesome — Amlogic 早期 H.264 偶有卡顿
        //   omx.hisi.video.decoder — HiSilicon 低版本硬解器
        return detectFlakyHardwareCodec();
    }
