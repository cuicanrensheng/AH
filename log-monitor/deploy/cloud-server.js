...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`
╔══════════════════════════════════════════════════════════════╗
║      TV Live Cloud Log Server                                ║
╠══════════════════════════════════════════════════════════════╣
║  🌐 HTTP API:   http://localhost:${PORT}                        ║
║  📡 WebSocket:   ws://localhost:${PORT}/ws                      ║
║  📝 日志接口:    POST /api/logs/:deviceId                     ║
║  📊 设备列表:    GET  /api/devices                           ║
║  🔍 日志查询:    GET  /api/logs/:deviceId                    ║
║  🗑️  清空日志:    DELETE /api/logs/:deviceId                 ║
║  ❤️  健康检查:    GET  /api/status                           ║
╠══════════════════════════════════════════════════════════════╣
║  已注册设备:  ${devices.size.toString().padStart(4, ' ')}                                    ║
║  缓冲日志数:  ${Array.from(logsBuffer.values()).reduce((sum, l) => sum + l.length, 0).toString().padStart(4, ' ')}                                    ║
║  已连接客户端:${clientConnections.size.toString().padStart(4, ' ')}                                    ║
╚══════════════════════════════════════════════════════════════╝
  `);
});
